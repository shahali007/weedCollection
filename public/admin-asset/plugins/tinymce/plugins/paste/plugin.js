 ...
 *  2. Wait for the browser to fire a "paste" record and get the contents out of the paste bin.
...
define("tinymce/pasteplugin/Clipboard", [
	...
	return function(editor) {
		...
		function pasteHtml(html) {
			...
			args = editor.fire('BeforePastePreProcess', {content: html}); // Internal record used by Quirks
		 ...
		 * so that when the real paste record occurs the contents gets inserted into this element
		 ...
		 *
		 * @param  {ClipboardEvent} e Paste/drop record object.
		 ...
		 *
		 * @param {Event} e Paste record object to check if it contains any data.
		...
		function registerEventHandlers() {
			editor.on('keydown', function(e) {
				...
				if (isKeyboardPasteEvent(e) && !e.isDefaultPrevented()) {
					...
					// IE doesn't support Ctrl+Shift+V and it doesn't even produce a paste record
					// so lets fake a paste record and let IE use the execCommand/dataTransfer methods
					...
					// Remove pastebin if we get a keyup and no paste record
					// For example pasting a file in IE 11 will not produce a paste record
